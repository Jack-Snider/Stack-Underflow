...
public class findId extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String regno = request.getParameter("InputregNoText");
		...
		if (name == "" || regno == "") {
			...
			MemberVO memVo1 = service.findregNo(regno);
			// 이메일을 입력받아 이름 존재시 정수를 받는다
			cnt=service.findregNoCount(regno);
			
			// 이메일을 검색하여 반환된 값이 1이면 compareEmail변수에 해당하는 이름을 넣어준다 
			// 그렇지 않다면 compareEmail="y"값을 준다
			if(cnt==0) {
				compareRegNo="y"; //이름과 이메일의 같은값일경우 발생하는 오류방지를 위한 임의문자선언(1)
			}
			else if(cnt==1) {
				compareRegNo = memVo1.getMem_nm();
			}
			...
			//  아이디를 출력한다
				if (compareName.equals(compareRegNo)) {
					out.print("<html>");
					out.print("<body>");
					out.print("<script>alert('당신의 아이디는 "+memVo1.getMem_id()+" 입니다.');");
					...
					if (compareName == "x" && compareRegNo=="y") {
						out.print("<html>");
						out.print("<body>");
						out.print("<script>alert('해당 이름과 주민번호가 존재하지 않습니다.');history.back();</script>");
						out.print("</body>");
						out.print("</html>");
					// 아이디 또는 이메일값이 정확할 경우 다시 입력하라고 알림을 띄워준다
						...
						out.print("<html>");
						out.print("<body>");
						out.print("<script>alert('이름과 주민번호가 일치하지 않습니다.');history.back();</script>");
						out.print("</body>");
						out.print("</html>");
					}
